...
const op_and      = new op((x, y) => ((x || x>0) && ( y || y>0))   ? bdds_base.T : bdds_base.F, '&&');
const op_and_not  = new op((x, y) => ((x || x>0) && (!y || y===0)) ? bdds_base.T : bdds_base.F, '&&!');
const op_or       = new op((x, y) => ((x || x>0) || ( y || y>0))   ? bdds_base.T : bdds_base.F, '||');
